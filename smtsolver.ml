...
let sat_type_from_string r input =
  if (String.compare r "sat" == 0) then Sat
    ...
    try
      let _ : int = Str.search_forward (Str.regexp "unexpected") r 0 in
...
let iget_answer2 chn input =
  ...
  let _ = List.map (fun s -> x_tinfo_pp s no_pos) model in
  let _ =
...
let iget_answer chn input =
  ...
  let st = if List.length output > 1 then
      try
        ...
        let _ = List.fold_left (fun old_b s -> old_b || (check_error_msg s)) false output in
